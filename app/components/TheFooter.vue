...
<template>
  <footer id="contact" class="bg-surface-muted border-t border-white/[0.06]">
    ...
    <div class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
      <div class="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div>
          ...
          <p class="mt-4 text-sm leading-relaxed text-ink-muted">{{ brand.tagline }}</p>
          <ul class="mt-5 space-y-2.5 text-sm">
            <li class="flex items-center gap-2.5">
              <PhoneIcon class="h-4 w-4 shrink-0 text-primary-400" />
              <a :href="`tel:${contact.phone.replace(/\s/g, '')}`" class="text-ink-muted transition-colors hover:text-white">{{ contact.phone }}</a>
            </li>
            <li class="flex items-center gap-2.5">
              <EnvelopeIcon class="h-4 w-4 shrink-0 text-primary-400" />
              <a :href="`mailto:${contact.email}`" class="text-ink-muted transition-colors hover:text-white">{{ contact.email }}</a>
            </li>
            <li class="flex items-start gap-2.5">
              <MapPinIcon class="mt-0.5 h-4 w-4 shrink-0 text-primary-400" />
              <span class="text-ink-muted">{{ contact.address }}</span>
            </li>
          </ul>
        </div>
        ...
        <nav aria-label="Quick links">
          ...
          <ul class="mt-4 space-y-2.5">
            <li v-for="link in footer.quickLinks" :key="link.label">
              <a :href="link.href" class="text-sm text-ink-muted transition-colors hover:text-white">{{ link.label }}</a>
            </li>
          </ul>
        </nav>
        ...
        <nav aria-label="Support">
          ...
          <div class="mt-6 flex gap-2.5">
            <a
              ...
              :title="item.name"
              class="flex h-9 w-9 items-center justify-center rounded-xl bg-surface-card ring-1 ring-white/10 transition-all hover:bg-primary-600 hover:ring-primary-500 hover:text-white text-ink-muted"
            >
    ...
    <div class="border-t border-white/[0.06]">
      <div class="mx-auto max-w-7xl px-4 py-5 text-center text-sm text-ink-subtle sm:px-6 lg:px-8">
        {{ footer.copyright }}
      </div>
    </div>
  </footer>
